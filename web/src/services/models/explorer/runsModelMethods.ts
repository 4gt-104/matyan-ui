...
const RUNS_TABLE_SORT_OPTIONS = [
  { value: 'run', label: 'run.name' },
  { value: 'experiment', label: 'run.experiment' },
  { value: 'hash', label: 'run.hash' },
  { value: 'date', label: 'run.creation_time' },
  { value: 'duration', label: 'run.duration' },
  { value: 'description', label: 'run.description' },
  { value: 'creator', label: 'run.creator' },
];

function getRunSortValue(m: any, field: string): string | number {
  const run = m?.run;
  if (!run?.props) return '';
  switch (field) {
    case 'run':
      return run.props.name ?? '';
    case 'experiment':
      return run.props.experiment?.name ?? 'default';
    case 'hash':
      return run.hash ?? '';
    case 'date':
      return run.props.creation_time ?? 0;
    case 'duration': {
      const start = run.props.creation_time ?? 0;
      const end = run.props.end_time ?? Date.now() / 1000;
      return end - start;
    }
    case 'description':
      return run.props?.description ?? '';
    case 'creator':
      return run.props?.creator?.username ?? '';
    default:
      return (run.props as any)?.[field] ?? '';
  }
}

function sortProcessedDataForRuns(
  data: any[],
  sortFields: SortFields | undefined,
): any[] {
  const effective = sortFields?.length
    ? sortFields
    : [{ value: 'run', label: 'Name', order: 'asc' as const }];
  const iteratees = effective.map(
    (f) => (m: any) => getRunSortValue(m, f.value),
  );
  const orders = effective.map((f) => f.order);
  return data.map((group: any) => ({
    ...group,
    data: _.orderBy(group.data || [], iteratees, orders),
  }));
}
...
function getRunsModelMethods(
  ...
  function getRunsData(
    ...
    const sortParam = sortFields?.length
      ? JSON.stringify(
          sortFields.map((f: any) => ({ field: f.value, order: f.order })),
        )
      : '';
    ...
    return {
      call: async () => {
        try {
          ...
          const sortFields = configData?.table?.sortFields;
          const sortedData = sortProcessedDataForRuns(data, sortFields);
  ...
  function updateModelData(
    ...
    const sortFields = configData?.table?.sortFields;
    const sortedData = sortProcessedDataForRuns(data, sortFields);
    const tableData = getDataAsTableRows(sortedData, metricsColumns, params);
  ...
  function onSortChangeHandler(arg: {
  ...
  function updateData(newData: any): void {
    ...
    const tableColumns = getRunsTableColumns(
      ...
      includeCreator,
      RUNS_TABLE_SORT_OPTIONS,
      modelState?.config?.table?.sortFields,
      onSortChangeHandler,
    );
  ...
  function finishRuns(ids: string[]): {
    call: () => Promise<void>;
    abort: () => void;
  } {
    runsFinishRef = runsService.finishRuns(ids);
    return {
      call: async () => {
        try {
          await runsFinishRef
            ...
            .then(() => {
              ...
              onNotificationAdd({
                notification: {
                  id: Date.now(),
                  severity: 'success',
                  messages: ['Runs are successfully stopped'],
                },
                model,
              });
            });
...
export { RUNS_TABLE_SORT_OPTIONS };
