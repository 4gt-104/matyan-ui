...
import { MATYAN64_ENCODING_PREFIX, encode } from 'utils/encoder/encoder';
...
function setDefaultAppConfigData(recoverTableState: boolean = true) {
  ...
  if (
    (searchParam.get('grouping') &&
      !searchParam.get('grouping')?.startsWith(MATYAN64_ENCODING_PREFIX)) ||
    (searchParam.get('chart') &&
      !searchParam.get('chart')?.startsWith(MATYAN64_ENCODING_PREFIX)) ||
    (searchParam.get('select') &&
      !searchParam.get('select')?.startsWith(MATYAN64_ENCODING_PREFIX))
  ) {
...
function getDataAsImageSet(
  ...
  if (!_.isEmpty(data)) {
    ...
    const groupFields = defaultGroupFields
      ? defaultGroupFields
      : configData?.grouping?.reverseMode?.row
      ? groupingSelectOptions
          .filter(
    ...
    Object.values(_.groupBy(metricsCollection.data, 'seqKey'))
      ...
      .forEach((metric: any) => {
        const rowValues: any = {
          ...
          isHidden: config?.table?.hiddenMetrics?.includes(metric.seqKey),
          index: rowIndex,
          color: metricsCollection.color ?? metric.color,
...
function onImagesSortChange({
  ...
  const newGroupingRow = resultSortFields
    .filter((f: any) => f.readonly)
    .map((f: any) => f.value);
  const currentGroupingRow = configData?.grouping?.row ?? [];
  const groupingRowChanged =
    newGroupingRow.length === currentGroupingRow.length &&
    newGroupingRow.some(
      (val: string, idx: number) => val !== currentGroupingRow[idx],
    );

  if (groupingRowChanged && configData) {
    const sortFieldsDict = resultSortFields.reduce((acc: any, f: any) => {
      acc[f.value] = f;
      return acc;
    }, {});
    const configUpdate = {
    ...
    updateModelData(configUpdate, true);
    analytics.trackEvent(
      `${ANALYTICS_EVENT_KEYS.images.imagesPanel.controls.changeSorting} Apply images sorting by a key`,
    );
  } else {
    updateImagesSortFields(
      resultSortFields,
      resultSortFields.reduce((acc: any, f: any) => {
        acc[f.value] = f;
        return acc;
      }, {}),
    );
        ...
        metricsKeys[0] === 'all'
          ? Object.values(processedData)
              .map((metricCollection: any) =>
                metricCollection.data.map((metric: any) => metric.seqKey),
              )
              .flat()
          : metricsKeys,
    };
