...
export function getDataAsMediaSetNestedObject<M extends State>({
  ...
  if (!_.isEmpty(data)) {
    ...
    data.forEach((group: any) => {
      const visibleData =
        hiddenMetrics.length > 0
          ? group.data.filter(
              (item: any) => !hiddenMetrics.includes(item.seqKey),
            )
          : group.data;
      ...
      const path = groupFields?.reduce(
        (acc: string[], field: string, index: number) => {
          const value = _.get(visibleData[0], field);
          _.set(
            orderedMap,
            acc.concat(['ordering']),
            new Set([
    ...
    return {
      mediaSetData: _.isEmpty(mediaSetData) ? fallbackData : mediaSetData,
      orderedMap,
    };
  }
  return {};
