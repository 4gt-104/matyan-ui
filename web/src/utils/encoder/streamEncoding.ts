...
const ArrayFlag = new MatyanObjectFlag('ARRAY_FLAG');
const ObjectFlag = new MatyanObjectFlag('OBJECT_FLAG');
...
type MatyanObjectPrimitive = null | boolean | number | string | ArrayBuffer;
type MatyanObjectArray = MatyanObject[];
type MatyanObjectDict = { [key: string]: MatyanObject };
type MatyanObjectNode = MatyanObjectDict | MatyanObjectArray;

type MatyanObject = MatyanObjectPrimitive | MatyanObjectNode;

type MatyanObjectKey = string | number;
type MatyanObjectPath = MatyanObjectKey[];

function valToNode(
  val: MatyanObjectPrimitive | MatyanObjectFlag,
): MatyanObject {
  if (val instanceof MatyanObjectFlag) {
    if (val === ObjectFlag) {
      return {};
    }
    if (val === ArrayFlag) {
...
export async function* iterFoldTree(
  pathsVals: AsyncGenerator<
    [MatyanObjectPath, MatyanObjectPrimitive | MatyanObjectFlag][]
  >,
  level: number = 0,
): AsyncGenerator<[MatyanObjectPath, MatyanObject | undefined]> {
  const stack: MatyanObject[] = [];
  const path: MatyanObjectPath = [];
  ...
  for await (let records of pathsVals) {
    ...
    for (let [keys, val] of records) {
      ...
      let keyToAdd: MatyanObjectKey = keys[keys.length - 1];
      path.push(keyToAdd);
      ...
      let lastState = stack[stack.length - 1] as MatyanObjectNode;
