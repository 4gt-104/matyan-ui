...
function SelectForm({
  ...
  function handleSelectAllVisible(): void {
    const selectedKeys = new Set(
      (selectedImagesData?.options || []).map((o: ISelectOption) => o.key),
    );
    const merged = [
      ...(selectedImagesData?.options || []),
      ...options.filter((o) => !selectedKeys.has(o.key)),
    ];
    onImagesExploreSelectChange(merged);
  }

  function handleDeselectAllVisible(): void {
    const visibleKeys = new Set(options.map((o) => o.key));
    onImagesExploreSelectChange(
      (selectedImagesData?.options || []).filter(
