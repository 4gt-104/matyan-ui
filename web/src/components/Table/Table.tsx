...
const Table = React.forwardRef(function Table(
  ...
  const [isOpenArchiveSelectedPopup, setIsOpenArchiveSelectedPopup] =
    React.useState(false);
  const [isOpenStopSelectedPopup, setIsOpenStopSelectedPopup] =
    React.useState(false);
  ...
  function onToggleStopPopup() {
    setIsOpenStopSelectedPopup(!isOpenStopSelectedPopup);
  }
  ...
  React.useEffect(() => {
    ...
    values.forEach((value) => {
      if (
        ...
        !tableBulkActionsVisibility.stop
      ) {
        if (value.archived) {
        ...
        if (value.end_time) {
          ...
          tableBulkActionsVisibility.stop = true;
        }
    ...
    <ErrorBoundary>
      {!isEmpty(rowData) ? (
        <div style={{ height: '100%', width: '100%' }} className={className}>
              ...
              {tableBulkActionsVisibility.stop && (
                <div className='selectedRowActionsContainer__selectedItemsDelete'>
                  <Button
                    ...
                    type='text'
                    size='small'
                    onClick={onToggleStopPopup}
                    className={`Table__header__item ${
                      isOpenStopSelectedPopup ? 'opened' : ''
                    }`}
                  >
          ...
          />
          <StopModal
            opened={isOpenStopSelectedPopup}
            onClose={onToggleStopPopup}
            selectedRows={selectedRows}
            onRowSelect={onRowSelect}
            finishRuns={finishRuns}
          />
        </div>
      ) : (
        <IllustrationBlock
