...
import SelectFormActions from 'components/SelectFormActions/SelectFormActions';
...
function QueryForm(props: IQueryFormProps) {
  ...
  const handleSelectAllVisible = React.useCallback(() => {
    const selectedKeys = new Set(
      (query?.selections || []).map((o: ISelectOption) => o.key),
    );
    const merged = [
      ...(query?.selections || []),
      ...options.filter((o) => !selectedKeys.has(o.key)),
    ];
    updateQuery.current({ selections: merged });
  }, [query?.selections, options]);

  const handleDeselectAllVisible = React.useCallback(() => {
    const visibleKeys = new Set(options.map((o) => o.key));
    updateQuery.current({
      selections: (query?.selections || []).filter(
        (s: ISelectOption) => !visibleKeys.has(s.key),
      ),
    });
  }, [query?.selections, options]);
            ...
            {query.advancedModeOn ? (
              ...
              <ErrorBoundary>
                <div className='flex fac QueryForm__topPanel'>
                  ...
                  >
                    <div className='QueryForm__Popper__content'>
                      <SelectFormActions
