...
import { MatyanFlatObjectBase } from 'types/core/MatyanObjects/MatyanFlatObjectBase';
...
export function storageDataToFlatList(
  ...
  const objectList: MatyanFlatObjectBase[] = []; // @CHECK make by hash function
  let params: string[] = [];
  ...
  runs.forEach((item) => {
    ...
    let collectedDataByDepth: Omit<MatyanFlatObjectBase, 'data'> = {};
    ...
    if (objectDepth === 0) {
      const object: MatyanFlatObjectBase = {
        key: buildObjectHash(objectHashCreator),
        ...collectedDataByDepth,
        data: depthInterceptor(item).data,
      };

      // If the object depth is Container, used run hash as a unique hash for MatyanObject
    ...
    if (objectDepth > 0) {
      ...
      item.traces.forEach((trace: any, traceIndex: number) => {
        ...
        // Generating unique hash creator obj for the MatyanObject using sequence name, context and container hash
        objectHashCreator = {
          ...objectHashCreator,
        ...
        if (objectDepth > 1) {
          ...
          trace.values.forEach((sequence: any, stepIndex: number) => {
            ...
            // Generating unique hash creator obj for the MatyanObject using sequence name, sequence context, step
            objectHashCreator = {
              ...objectHashCreator,
              ...record_data,
            };
