...
import { MatyanErrorType, SelectorCreator } from 'modules/core/engine/types';

import { SequenceTypesEnum } from 'types/core/enums';

type Status = {
  isLoading: boolean;
  error: MatyanErrorType | null;
};
...
> = {
  ...
  setError: (error: MatyanErrorType | null) => void;
  getStatus: () => Status;
...
function createState<TStore, SequenceName extends SequenceTypesEnum>(
  ...
  > = {
    ...
    setError: (error: MatyanErrorType | null = null): void => {
      store.setState(
        produce<Store>((draft_state: Draft<Store>) => {
          draft_state.instructions.status.isLoading = false;
