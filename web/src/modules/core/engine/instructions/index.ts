...
import MatyanError from 'modules/core/MatyanError';
  ...
  const getInstructions = (): Promise<boolean> => {
    return new Promise((resolve, reject) => {
      getParams({ sequence: options.sequenceName })
        ...
        .catch((e) => {
          const matyanError = new MatyanError(
            e.message || e,
            e.detail,
          ).getError();
          state.setError(matyanError);
          reject(matyanError);
          notificationsEngine?.error(matyanError.message);
        });
    });
  };
